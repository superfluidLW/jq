...
public class MonteCarloDKoCalculator extends OptionCalculator {
  ...
  public Double calcPv(Deal trade, Market market) {
    ...
    Double spot = market.tickerPrice(option.getUnderlying());
    LocalDate exerciseDate = ((DoubleBarrierOption) trade).getExerciseDate();
    Double volatility = market.tickerVol(option.getUnderlying());

    Double yearFrac = DayCountImpl.get(option.getDayCount()).yearFraction(market.getMktDate(), exerciseDate);
    Double riskFreeRate = market.jqCurve(option.getDomCurrency()).getZeroRate(yearFrac);
    Double discFact = market.jqCurve(option.getDomCurrency()).getDf(yearFrac);
    ...
    for (int i = 0; i < nPaths; ++i)
    {
      ...
      if (!isOut)
      {
        ...
        pv = option.getOptionDirection() == OptionDirection.Call
