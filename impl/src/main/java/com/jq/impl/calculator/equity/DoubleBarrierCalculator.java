...
public class DoubleBarrierCalculator extends OptionCalculator {
    ...
    public Double calcPv(Deal trade, Market market){
        BarrierOptionFourierPvPricer pvPricer = new BarrierOptionFourierPvPricer((DoubleBarrierOption) trade, market);
...
class BarrierOptionFourierPvPricer
{
    ...
    public BarrierOptionFourierPvPricer(DoubleBarrierOption option, Market market)
    {
        ...
        Double riskFreeRate = market.jqCurve(option.getDomCurrency()).getZeroRate(exerciseTime);
        Double spotPrice = market.tickerPrice(option.getUnderlying());
        Double vol = market.tickerVol(option.getUnderlying());
        Double exerciseInYears = DayCountImpl.get(option.getDayCount()).yearFraction(market.getMktDate(), exerciseDate);

        _optionType = option.getOptionDirection();
        _rebate = option.getKoRebate();
        _coupon = option.getKiCoupon();
        _K = option.getStrike();
        _S = spotPrice;
        _T = exerciseInYears;
        _v = vol;
        _r = riskFreeRate;
        _B = option.getUBarrier();
        _A = option.getLBarrier();
        _participationRate = option.getParticipationRate();
...
class BarrierOptionPvPricer {
    ...
    public BarrierOptionPvPricer(DoubleBarrierOption option, Market market) {
        LocalDate exerciseDate = option.getExerciseDate();
        Double exerciseTime = DayCountImpl.get(option.getDayCount()).yearFraction(market.getMktDate(), exerciseDate);
        Double riskFreeRate = market.jqCurve(option.getDomCurrency()).getZeroRate(exerciseTime);
        Double spotPrice = market.tickerPrice(option.getUnderlying());
